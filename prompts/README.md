...
| [`move_around_tool.md`](move_around_tool.md) | the `move_around` tool (`harp/motion/tools.py`, only when harp.yaml `motion.enabled`) | The tool's `description` field — what teaches the model it can drive the base on one bounded stall-patrol lap when asked, and to stop the moment anyone says stop. |
| [`follow_tool.md`](follow_tool.md) | the `follow_person` tool (`harp/motion/tools.py`, only when harp.yaml `motion.enabled` and the camera + face-ID are running) | The tool's `description` field — what teaches the model it can follow a person face-ID *recognizes* (strangers get an error), to warn about safe distance first, and to stop the moment anyone says stop. |
