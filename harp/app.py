...
the motors at once. The servo HEAD's face tracking (harp/motion/head_tracker,
harp.yaml `motion.gimbal_*`) is wired here too when enabled: it reads the same
shared camera as everything else (no second RealSense) to keep a face centered,
and optionally pops the fullscreen face page open at boot.
...
async def run_app(provider_name: str = "gemini") -> None:
    ...
    # Head gimbal face tracking (harp.yaml motion.gimbal_*): the servo head
    # keeps a detected face centered, reading the SAME shared camera as
    # everything else — no second RealSense open. Needs the camera (without it
    ...
    if settings.motion.gimbal_enabled:
        if camera is None:
            logger.warning(
                "head tracking: no camera this run — the gimbal would only hold "
                "still, so it's disabled"
            )
        else:
            try:
