...
- **Voice/noise tuning — one dashboard panel, both modes.** [harp/config.py](harp/config.py)'s
  ...
  whichever agent currently owns the microphone — the plain single-agent
  `VoiceBridge` (default) or, when `filter_agent.enabled`, the two-agent
  filter. The dashboard's "Voice tuning" panel (loudness gate, VAD
  threshold/silence, noise reduction) is wired in every run, not just
  two-agent mode. See the 2026-07-08 log entry.
...
- **Per-run session log — the developer log.** [core/session_log.py](harp/core/session_log.py)
  ...
  run ACTUALLY used, every bus event (state, wakes, transcript, tools,
  detections, errors, heartbeats), and every internal log line, flushed per
  line so a crash keeps everything up to the moment it happened (no
  `session_end` record = the run died there). Old runs pruned at startup;
  configure/disable via harp.yaml `session_log:`. Read it to debug a past run
  cold. Tests: [tests/test_session_log.py](tests/test_session_log.py) (5).
- **Long-term memory — HARP remembers the people it talks to.**
  [harp/memory/](harp/memory/) is now fully implemented around one **parallel
  Gemini Flash Lite helper agent** ([memory/agent.py](harp/memory/agent.py),
  harp.yaml `memory:`, needs `GEMINI_API_KEY`) behind a shared sliding-window
  rate cap (14 calls/min — the free-tier budget). Three flows: **(1)** every
  conversation is recorded turn-by-turn ([memory/logger.py](harp/memory/logger.py),
  `.harp/memory/interactions/`, crash-safe `.part`→`.jsonl` lifecycle),
  mechanically digested ([memory/parse.py](harp/memory/parse.py)), then
  summarized ([memory/summarizer.py](harp/memory/summarizer.py)) into
  `{summary, follow_up, person_facts}` attached to EVERY enrolled participant;
  unknown-visitor conversations land in `.harp/memory/guestbook.jsonl` instead
  (no face stored, transcript filename kept so a later enrollment can
  re-attach). A failed model call leaves the transcript pending and a boot
  sweep retries — a quota outage delays memory rather than losing it.
  **(2)** A **pre-computed wake briefing** ([memory/context.py](harp/memory/context.py)):
  when face-ID sees someone while idle, the helper fuses the camera frame +
  their stored memories into a "who you're about to talk to" paragraph,
  cached and refreshed on people-change or every `context_ttl_seconds`
  (120 s) — delivered through app.py's existing `identity_context` seam at
  session open, so it costs zero wake latency; the static face-ID line stays
  as fallback. **(3)** Mid-session tools for the live model: `search_memory`
  ([memory/tools.py](harp/memory/tools.py) — keyword search over all
  summaries + notes + guestbook, works even without the helper) and
  `describe_scene` ([vision/describe.py](harp/vision/describe.py) — the
  helper describes the current frame; needs helper + camera). New events
  `MemoryWritten`/`ContextPrepared` appear on the dashboard/session log with
  no changes there. See the 2026-07-09 log entry. **Not yet verified live.**
...
- **core/** — the spine everything else depends on: `bus.py` (async pub/sub,
  ...
  `session_log.py` (per-run dev log, ✅ implemented + tested — see above).
...
- **memory/** — ✅ ALL implemented + tested: `store.py` + `matcher.py` (JSON
  per person in `.harp/memory/people/`, cosine matcher, enrollment via
  [scripts/enroll_people.py](scripts/enroll_people.py) over `people/`), and —
  new 2026-07-09 — `agent.py` (the shared Flash Lite helper + rate limiter),
  `logger.py`, `parse.py`, `summarizer.py`, `context.py`, `tools.py` (see the
  long-term-memory bullet above).
...
3. **Live verification of long-term memory (user, needs GEMINI_API_KEY +
   camera):** have a conversation as an enrolled person and check (a) a
   transcript appears in `.harp/memory/interactions/` and flips to
   `.jsonl.done` shortly after the session ends, (b) the summary lands in your
   `.harp/memory/people/<id>.json`, (c) stand in frame while idle and watch
   `ContextPrepared` on the dashboard, then wake it — HARP should greet you
   with your history, (d) ask "what can you see?" (describe_scene) and "do you
   remember me?" (search_memory), (e) a stranger's conversation lands in
   `.harp/memory/guestbook.jsonl`. **Also confirm the helper model id** —
   harp.yaml `memory.model` says `gemini-3.1-flash-lite`; if the API rejects
   it, every helper call becomes a logged no-op (watch for "memory agent:
   generate failed" in the session log) — set the correct id there.
4. Then PLAN.md's remaining order: a dashboard "delete logs" button, the
   watchdog (last piece of phase 2), web-search fallback, **richer proactive
   triggers** — now unblocked, since stored memories carry `follow_up` intents
   the trigger engine can match against a recognized returning face — and a
   silence-based end rule (a natural companion to the face-absence rule).
...
### 2026-07-09 — Long-term memory: a parallel Flash Lite helper (PLAN phase 6)

The memory phase, built to the shape brainstormed and decided with the user
this session: **pre-compute on sighting** (not on wake — a wake must never
wait on a model call); **14 helper calls/min** hard cap (free tier);
regenerate the wake briefing when **who is in frame changes or 2 minutes
pass**; **unknown-visitor interactions are remembered too** (guestbook — no
face stored, per the 2026-07-02 decision; enrollment of unknowns is a later
challenge); summaries attach to **all** enrolled participants; mid-session
the live model gets **describe_scene** and **search_memory** tools instead of
late context injection; **face-ID is the only person source** (no new
detector).

- **One shared helper** ([memory/agent.py](harp/memory/agent.py)):
  `GeminiAgent` (gemini-3.1-flash-lite, harp.yaml `memory.model`) +
  `RateLimiter` (sliding 60 s window shared by all three consumers).
  Fail-safe by contract: `generate()` returns None on ANY failure (rate cap,
  SDK error, timeout, missing key) — consumers each have a defined
  degradation, nothing can raise into the runners. Background callers skip
  when the window is full; callers a model is waiting on block for a slot.
- **Transcript logger** ([memory/logger.py](harp/memory/logger.py), the old
  stub, now real): one JSONL per conversation in `.harp/memory/interactions/`
  — start/person/turn/tool/end records (finals only, no deltas),
  participants seeded at open from the new `FaceID.people_now()` (the bus
  doesn't replay pre-wake sightings). Crash-safe: flushed per line, `.part`
  while live → `.jsonl` on close (also on app shutdown mid-conversation);
  `rescue_stale_transcripts()` promotes leftovers at boot.
- **Deterministic parse** ([memory/parse.py](harp/memory/parse.py)) — the
  user's "script parses some info before calling Gemini": who/when/why-woke/
  why-ended/duration/turn-count/search_knowledge-queries extracted
  mechanically and handed to the prompt as facts; owns the record vocabulary.
- **Summarizer** ([memory/summarizer.py](harp/memory/summarizer.py), the old
  stub, now real): on InteractionEnded (plus a boot sweep), digest → one
  JSON-mode helper call → `{summary, follow_up, person_facts}` →
  `store.add_summary` for every enrolled participant (store entries carry the
  extra fields now), or a guestbook line (with transcript filename for later
  re-attachment) when nobody was enrolled. No-conversation wakes are skipped
  without a call; a failed call leaves the file pending for the next sweep —
  quota loss delays memories, never loses them. `MemoryWritten` on the bus.
- **Wake briefing** ([memory/context.py](harp/memory/context.py)): on
  PersonIdentified while STANDBY, fuse the clean camera frame (no overlay
  boxes) + store history into a briefing, cached; refreshed on
  people/head-count change and TTL lapse; cleared on empty frame; loop stops
  during ACTIVE (no mid-session quota burn). Consumed through app.py's
  existing `identity_context` seam — bridge/orchestrator untouched, static
  face-ID line stays as the fallback. `ContextPrepared` on the bus.
- **Tools**: `search_memory` ([memory/tools.py](harp/memory/tools.py)) —
  token-overlap ranking (BM25 would be ceremony at dozens of entries;
  knowledge/retriever's tokenizer made public and reused) over summaries +
  enrollment notes + guestbook, declared whenever memory is enabled (needs no
  API key); `describe_scene` ([vision/describe.py](harp/vision/describe.py))
  — helper describes the current frame mid-session, declared only when
  helper + camera exist. Both mirror knowledge/tools.py's declaration shape
  and error-payload-not-exception contract.
- **Config/prompts**: harp.yaml `memory:` block (`enabled`/`model`/
  `calls_per_minute`/`context_ttl_seconds`) + `MemorySettings`; five new
  prompt files (memory_summarizer, context_writer, describe_scene,
  describe_scene_tool, search_memory_tool) indexed in prompts/README.md, all
  with code fallbacks. app.py wires everything; without `GEMINI_API_KEY` the
  transcripts still record (summarized by a later run that has the key) and a
  warning says so.
- Tests: +25 across [test_memory_agent.py](tests/test_memory_agent.py) (rate
  window slides, acquire waits/timeouts, generate never raises, budget skip,
  fenced-JSON parse), [test_memory_parse.py](tests/test_memory_parse.py)
  (realistic + crashed digests), [test_interaction_logger.py](tests/test_interaction_logger.py)
  (finals-only + seeding, shutdown finalizes, rescue), [test_summarizer.py](tests/test_summarizer.py)
  (all-participants attach, guestbook + malformed-JSON degrade, empty-skip
  without a call, pending-on-failure retry, boot rescue + event sweep),
  [test_context_writer.py](tests/test_context_writer.py) (ready-before-wake,
  people-change regen + stale-key serves nothing, TTL refresh, no refresh in
  ACTIVE + absence clears, failure serves fallback), [test_memory_tools.py](tests/test_memory_tools.py)
  (name-over-body ranking, three corpora, describe_scene error payloads), +2
  config (yaml drift guard, defaults/override). **Full suite: 213 pass**
  (was 188). Found while testing: the refresh-loop deadline had a 1 s floor
  that silently overrode sub-second TTLs — floor is now 0.05 s (spin guard
  only).
- **NOT verified live** (needs GEMINI_API_KEY + camera + a real face): see
  "Suggested next steps" #3 — including confirming the `gemini-3.1-flash-lite`
  model id against the real API.

### 2026-07-08 — Wake/identity context also moved out of code, into prompts/

Follow-up to the same-day "every agent prompt moved out of code" entry below:
that pass covered personas and tool descriptions, but missed a second family
of model-facing text — the **context delivered into the live session at
open**, explaining why HARP just woke and (when face-ID recognizes someone)
who it's talking to. Five more literal strings, in four different files:

- `harp/app.py::identity_context()` — the face-ID "you are talking to
  `<name>`" line (two variants: with/without notes).
- `harp/triggers/engine.py::_WAVE_CONTEXT` — sent when a wave wakes HARP.
- `harp/interaction/push_to_talk.py::_PTT_CONTEXT` — sent when push-to-talk
  wakes HARP.
- `harp/listener/listener.py` — two inline f-strings, for the "loud sound"
  and "wake word" wake reasons.

Same treatment: moved to `prompts/identity_context.md`,
`prompts/identity_context_with_notes.md`, `prompts/wake_context_wave.md`,
`prompts/wake_context_push_to_talk.md`, `prompts/wake_context_loud_sound.md`,
`prompts/wake_context_wake_word.md` — all indexed in
[prompts/README.md](prompts/README.md). Three of these are templates
(`{name}`/`{notes}`, `{level}`, `{text}`); a new `format_prompt(template,
fallback, **kwargs)` in [harp/config.py](harp/config.py) fills them in and
falls back to the built-in default text if a hand-edited template drops a
placeholder the code expects, rather than crashing the listener/bridge
mid-wake. `harp.yaml`'s prompts index and `README.md`'s "Agent prompts"
section both updated to mention this second category. No behavior change —
each `.md` file's body matches the text it replaced. Full suite unaffected.

### 2026-07-08 — Every agent prompt moved out of code, into prompts/

First stage of the logging system: a per-run DEVELOPER log a later debugging
session (person or agent) can read cold. One append-only JSONL file per run of
the full agent — `.harp/logs/session-<timestamp>.jsonl` — merging three sources
into a single time-ordered timeline (scheme discussed + agreed with the user
before building: JSONL over plain text, everything logged including streaming
deltas and heartbeats, newest-30-runs retention):

- **Header** (`session_start`): the effective merged settings, provider +
  resolved model/voice, platform/python — what the run's knobs ACTUALLY were,
  not what harp.yaml says today.
- **Every bus event** verbatim via a no-filter subscription (event types added
  later are captured with no logger change — the dashboard raw-view trick):
  state changes, wakes, the conversation (UserSaid/AgentSaid incl. deltas),
  tools, detections, errors, heartbeats (a gap in heartbeats = the stall
  window).
- **Every Python logging record** via a handler on the root logger — the
  camera warnings / mic retries / "filter heard (asr)" lines that carry most
  of the debugging story and used to vanish with the terminal.
  `logger.exception` records keep their traceback (`exc` field).

[core/session_log.py](harp/core/session_log.py) (`SessionLog`). Fail-safe by
design (the status_voice philosophy): unserializable payloads degrade to repr,
a write error drops the line, records after close are ignored — it can never
take the agent down (app.py treats a finished task as a crash). Each line is
flushed as written, so a crash preserves the record up to the moment it
happened; a file with no `session_end` record IS the diagnostic (the run
died). Thread-safe (bus events on the loop; log records from camera / pynput /
audio threads). Prunes to the newest `keep_runs` files at startup. New
`session_log:` block in [harp.yaml](harp.yaml) + `SessionLogSettings`
([config.py](harp/config.py)); wired in [app.py](harp/app.py) FIRST — before
the camera open — so even the earliest boot warnings land, and its runner is
first in the dict so its subscription registers before the orchestrator's
first events.

- Tests ([tests/test_session_log.py](tests/test_session_log.py), 5): the mixed
  real load (4 threads logging while bus events flow — every line parses, none
  lost), unserializable-payload repr fallback, dying-disk + after-close writes
  never raise, prune deletes only the oldest runs, header/events/logs/traceback
  round-trip. +1 guard in test_config.py (a typo'd top-level yaml key would
  silently disable the log). **Full suite: 188 pass.**
- **Verified live** (camera stubbed to the unavailable path): a real 8 s boot
  of `run_app("gemini")` produced 11 records — header, camera-unavailable
  warning, status-voice no-clip warnings, StateChanged starting→standby, 2
  Heartbeats, session_end — all parseable.
- **Found while verifying — pre-existing, NOT fixed here:** a real
  `cv2.VideoCapture(0)` open can hang indefinitely on this machine (OS-level
  camera block, sibling of the 2026-07-06 mic block; it runs in the default
  executor, so boot never proceeds and the process can't exit — the executor
  join at shutdown waits forever). The session log made the freeze legible:
  that run's file ends right after "session log: ..." with no camera warning →
  stuck inside the camera open. A timeout around the camera open is a good
  next vision-robustness step.
- Stage 2 of logging (separate, still a stub): the per-person interaction
  transcript for the memory summarizer ([memory/logger.py](harp/memory/logger.py))
  — different consumer and lifetime; the session log deliberately doesn't try
  to be it.

### 2026-07-08 — Every agent prompt moved out of code, into prompts/

The main persona and filter persona already lived in `prompts/*.md`
(`load_persona`/`load_filter_persona`), but four more pieces of text actually
sent to a model were still hardcoded string literals: the `search_knowledge`
and `end_session` tool *descriptions* (the wording that teaches the model
when/how to call each tool) and the two transcription-steering prompts
(OpenAI's input transcriber, the local wake-word Whisper). None of this was
discoverable from `harp.yaml` either — a non-developer editing settings would
never learn `prompts/` existed.

- **New prompt files**: [prompts/search_knowledge_tool.md](prompts/search_knowledge_tool.md),
  [prompts/end_session_tool.md](prompts/end_session_tool.md),
  [prompts/transcription_openai.md](prompts/transcription_openai.md),
  [prompts/transcription_whisper.md](prompts/transcription_whisper.md) — same
  "> note" / "# title" authoring-notes convention as the existing personas.
- **[prompts/README.md](prompts/README.md)** (new): one table indexing all six
  prompt files — which agent/tool consumes each, and which env var overrides
  it if any.
- **[harp/config.py](harp/config.py)**: `load_persona`/`load_filter_persona`
  refactored onto a shared `load_prompt(path, fallback)` (same stripping logic,
  no longer duplicated); four new loaders (`load_search_tool_description`,
  `load_end_session_description`, `load_openai_transcribe_prompt`,
  `load_whisper_prompt`) with their own `FALLBACK_*` constants, mirroring the
  existing persona pattern. A missing/emptied prompt file degrades to the
  fallback instead of failing to start.
- **Call sites now load instead of hardcode**:
  [knowledge/tools.py](harp/knowledge/tools.py), [interaction/session_tools.py](harp/interaction/session_tools.py),
  [voice/openai.py](harp/voice/openai.py) (`_DEFAULT_TRANSCRIBE_PROMPT`),
  [listener/transcriber.py](harp/listener/transcriber.py) (`_ROMAN_URDU_PROMPT`).
  The `OPENAI_TRANSCRIBE_PROMPT`/`HARP_WHISPER_PROMPT` env-var overrides still
  work exactly as before, layered on top of the (now file-backed) default.
- **[harp.yaml](harp.yaml)** gained a comment block at the top pointing to
  `prompts/README.md`, and **[README.md](README.md)** gained an "Agent
  prompts" section under Settings — so both places a user would look for
  "how do I change what HARP says" now point at `prompts/`.
- No behavior change: every new `.md` file's body is byte-for-byte the text
  that used to be the hardcoded literal, so the model sees identical
  instructions. Full test suite unaffected (no test asserted the literal
  description/prompt text, only shape).

### 2026-07-08 — Voice/noise tuning generalized to single-agent mode

The dashboard's noise/VAD tuning panel (loudness gate, VAD threshold/silence,
noise reduction) only existed for the two-agent filter — `filter_agent.enabled:
false` (the ordinary, non-experimental path) got no dashboard control over
noise at all, even though the exact same problem (a native-audio model
committing/hallucinating a turn from room noise) applies to its own mic. Made
the whole mechanism mode-agnostic instead of filter-specific:

- **`LoudnessGate` extracted** to [voice/loudness_gate.py](harp/voice/loudness_gate.py)
  (was private to filter_agent.py), now reusing
  [listener/detector.py](harp/listener/detector.py)'s `rms_level` instead of a
  second copy of the same formula. Re-exported from `filter_agent` so existing
  imports/tests didn't need to change.
- **`VoiceBridge` (the plain single-agent session) now runs the gate too**
  ([voice/bridge.py](harp/voice/bridge.py)): a new `near_field_level` param,
  applied in `_pump_mic` after the push-to-talk gate — same "two gates in
  series" shape `FilterAgent._pump_mic` already used. Default `None` = off,
  identical behavior to before.
- **Config renamed, not just extended**: `FilterAgentSettings`
  (`near_field_level`/`vad_threshold`/`vad_silence_ms`/`noise_reduction`) is
  now `VoiceTuningSettings` under a new top-level `harp.yaml` `voice_tuning:`
  section — `filter_agent:` keeps only `enabled`/`provider`/
  `response_tail_seconds`. The runtime `FilterTuning` dataclass is now
  `VoiceTuning` (config.py); nothing in its shape was filter-specific to begin
  with. `build_session_config(provider, tuning=...)` gained the same
  VAD-stamping `build_filter_config` already did (which now just calls it).
- **Dashboard wiring un-gated**: `app.py` builds one `voice_tuning =
  VoiceTuning(...)` and always passes `set_voice_tuning`/`get_voice_tuning`
  to `serve_dashboard(...)` — previously `None` unless `filter_agent.enabled`.
  Event renamed `FilterTuningChanged` → `VoiceTuningChanged`, message
  `SetFilterTuning` → `SetVoiceTuning` (core/events.py, dashboard/server.py).
  Frontend panel renamed "Filter tuning (two-agent noise filter)" → "Voice
  tuning (noise / VAD, whichever agent owns the mic)"
  ([static/index.html](harp/dashboard/static/index.html),
  [static/app.js](harp/dashboard/static/app.js)); same sliders, same
  confirmation-based apply/broadcast flow, no other frontend logic changed.
- In two-agent mode nothing behaviorally changes — `voice_tuning` is the same
  object the filter's `near_field_level`/`build_filter_config` already
  consumed, just no longer filter-only by construction.
- Tests: renamed/updated `test_config.py`, `test_dashboard.py`,
  `test_filter_agent.py` for the new names; new
  `test_build_session_config_stamps_tuning_for_the_single_agent_session` (the
  behavior that actually matters — the single-agent session's own
  `SessionConfig` picks up the VAD knobs) and two new `test_bridge.py` cases
  (`test_mic_pump_applies_the_loudness_gate` — the gate is really wired into
  `_pump_mic`, mirroring the filter's own test; a default-off regression
  check). **Full suite: 183 pass.** Not yet verified live — same "needs
  calibration against the real mic/room" caveat as the 2026-07-06 entry below.
