...
\subsubsection{Acceptance of Assistive Robots}

Finally, whether such robots are actually welcome is an empirical question with a growing evidence base. A systematic review of socially assistive humanoid robots identifies enjoyment, usability, personalization, and familiarity as the main enablers of acceptance, and technical problems, limited capability, and negative preconceptions as the main barriers. Surveys of staff across service settings find genuine openness to robots taking over routine tasks, alongside concerns about privacy and effects on their own roles. Users are broadly positive but prefer robots to act as assistants rather than companions or decision-makers. Reviews of deployed service robots report promising outcomes while cautioning that methodological quality limits firm conclusions. For HARP, these findings translated into concrete design priorities: personalization (greeting known people by name), reliability, a clearly assistive rather than autonomous role, and privacy safeguards such as never storing the faces of unknown people.
...
\item To deliver operational efficiency and customer satisfaction by delivering consistent, personalized, and uninterrupted services through AI-driven learning and decision-making.
...
Figure~\ref{fig:dataflow} shows how these modules connect at run time: sensing and user input feed the Jetson Nano perception and speech processing and the Raspberry Pi 5 path planner, which produce user gender, age, and emotion estimates, spoken responses, and navigation commands.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{flowchart.png}
\caption{High-level data flow across HARP: RealSense perception and user prompts feed the Jetson Nano perception and speech processing and the Raspberry Pi 5 path planner, which produce user gender, age, and emotion estimates, spoken responses, and navigation commands.}
\label{fig:dataflow}
\end{figure}
...
\subsubsection{Face Detection with Depth, Gender, Emotion, and Gesture Recognition}
...
The engineering methodology behind this software is a project outcome in its own right. The hardest unknown, real-time bilingual voice quality, was proven in a throwaway spike before any architecture was committed. 
...
\subsection{Socio-Economic Impact of the Project}
...
\subsection{Efforts towards Marketing, Commercialization, and Linkages}
\label{sec:linkages}

To commercialize HARP, the team is targeting private and public-sector hospitals through pilot deployments and strategic partnerships. Demonstrations highlighting patient assistance, emotion recognition, and multilingual interaction are being used to show gains in operational efficiency and patient experience. To support scaling and deployment, research proposals have been submitted under the RASTA/PIDE and NEC NRPU 2025 funding schemes. Public exposure at trade expos and competitions (Figure~\ref{fig:showcase}) has already produced an international research collaboration, and commercialization discussions with JS Bank continue in parallel. Table~\ref{tab:linkages} summarizes the project's public exposure, funding progress, and linkages with R\&D organizations, universities, and industry.

\begin{table}[H]
\centering
\begin{tabular}{@{}L{3.8cm}L{3.4cm}L{7.3cm}@{}}
\toprule
\textbf{Engagement} & \textbf{Type} & \textbf{Outcome / Status} \\
\midrule
\textbf{ASEAN}-Pakistan Trade and Investment Expo (\textbf{APTEX}) & Exhibition / showcase & Led to an international research collaboration with \textbf{Dr.\ Daphne Teck Ching Lai} (Brunei Darussalam) \\
\addlinespace
\textbf{COMPPEC} & Exhibition / showcase & Publicly demonstrated to students, academia, and industry visitors \\
\addlinespace
\textbf{Indus RAS} Expo, 22-23 July 2026 & Exhibition / showcase & Upcoming showcase \\
\addlinespace
\textbf{RASTA/PIDE} research proposal & Research funding & Joint proposal with Dr.\ Daphne Teck Ching Lai (Brunei Darussalam) submitted \\
\addlinespace
HEC \textbf{NRPU} 2025 funding scheme & Research funding & Reached stage two of the evaluation \\
\addlinespace
\textbf{JS Bank} & Industry / commercialization & Two online meetings held \\
\bottomrule
\end{tabular}
\caption{Marketing exposure, commercialization progress, and linkages with R\&D organizations, universities, and industry.}
\label{tab:linkages}
\end{table}
...
\centering
\includegraphics[trim={16cm 0 0 0},clip,width=\linewidth,height=0.32\textheight,keepaspectratio]{ASEAN_Showcase.jpeg}
\caption{}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.35\textwidth}
\centering
\includegraphics[trim={0 0 0 28cm},clip,width=\linewidth,height=0.32\textheight,keepaspectratio]{COMPPEC_Showcase.jpeg}
\caption{}
\end{subfigure}
\caption{HARP on public display: (a) at the ASEAN-Pakistan Trade and Investment Expo (APTEX), and (b) at the All-Pakistan Inter-Colleges/Universities Computer Project Competition (COMPPEC).}
\label{fig:showcase}
\end{figure}
...
\midrule
\textbf{Journal paper} & One journal paper under review, submitted to Advanced Mechatronics (Taylor and Francis, W category) \\
\textbf{Patent / IP} & Mechanical design patent filed at the NUST Intellectual Property Office \\
\textbf{Product type} & Working prototype (design model, software, and integrated hardware platform) \\
\textbf{Software} & Two open codebases: the ROS 2 robot control stack and the modular conversational agent, plus the student teams' repositories (including the FYDP ROS 2 environment on GitHub) \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
\textbf{Category} & \textbf{Team} & \textbf{Focus} \\
\midrule
FYDP 2023-24 (DE-42 MTS) & Hassan Rizwan, Numan Siddique, Ifrah Sajjad, Fiza Hassan & Mechanical design and fabrication of the torso, head, and neck; emotion and gender recognition; Gemini chatbot with RAG; web user interface \\
\addlinespace
FYDP 2024-25 (DE-43 MTS) & Syed Muhammad Daniyal Gillani, Muhammad Abdullah Khalid, Wajieh Badar, Umair Shahzad & Omnidirectional mecanum base design and fabrication; 2 DoF neck gimbal; behavior recognition; speech pipeline; passive visual SLAM; ROS 2 integration \\
\addlinespace
FYDP 2025-26 (DE-44 MTS) & Noor-Ul-Ain, Aqib Ali & IoT administrative control and web dashboard; visitor logging; YOLOv8 object detection, tracking, and segmentation; MediaPipe gesture control; Gemini Live integration \\
\addlinespace
Internship (2025) & Muhammad Zahid, Abdullah Virk, Hassan Ali Ahmed, Uzair Ali and Muhammad Talha & VLM based path planning \\
\addlinespace
Internship (2026) & Sheih Muhammad Ismail, Asif Mujtaba, Uswah Jafar and Abdul Rafay & Enhancing HARP Perception Module \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Reasons for Project Extensions}
...
A second, technical challenge was the on-board computing platform. The NVIDIA Jetson Nano does not have enough memory or processing power to run local large language models effectively. Even where a smaller, quantized model does fit, its quality falls well short of what frontier models deliver through cloud API calls. A more capable board such as the NVIDIA Jetson Orin would have performed considerably better here, but the project's focus was on developing the hardware platform, and the limited budget was spent mostly on that. HARP therefore relies on API-based conversational intelligence rather than fully on-device language models, which improves response quality.
...
\item \textbf{On-device language models.} Upgrade the on-board computer to a more capable platform, such as the NVIDIA Jetson Orin, so that capable local language models can run directly on the robot.
